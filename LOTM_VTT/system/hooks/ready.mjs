...
function registerSockets() {
    game.socket.on("system.LOTM_VTT", (data) => {
...
async function _handlePrompt(message) {
    ...
    const respond = (data) => {
        if (responded) return;
        responded = true;
        game.socket.emit("system.LOTM_VTT", {
            type: "promptResponse",
            uuid: message.uuid,
            data: data ?? {}
...
function reopenLastState() {
    const lastState = game.settings.get("LOTM_VTT", "hotReloadLastState");
    ...
    game.settings.set("LOTM_VTT", "hotReloadLastState", { openWindows: [] });
}
...
async function createKeywordJournals() {
    ...
    if (!game.settings.get("LOTM_VTT", "createSystemJournal")) return;
    ...
    let keywordsJournal = game.journal.find(j => j.getFlag('LOTM_VTT', 'systemJournal') === true);
