// node_modules/vuetify/dist/vuetify-labs.esm.js
...
var VSlideGroup = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    function onFocus(e) {
      if (!ignoreFocusEvent && !isFocused.value && !(e.relatedTarget && contentRef.el?.contains(e.relatedTarget)))
        focus();
...
function useVirtual(props, items) {
  ...
  const computedItems = computed(() => {
    return items.value.slice(first.value, last.value).map((item, index2) => ({
      ...
      key: isObject(item) && "value" in item ? item.value : index2 + first.value
    }));
  });
  watch(items, () => {
    sizes = Array.from({
...
var VSelect = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const menu = computed({
      get: () => _menu.value,
      set: (v) => {
        if (_menu.value && !v && vMenuRef.value?.\u03A8openChildren.size)
          return;
        _menu.value = v;
      }
    });
...
var VRating = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    function VRatingItem(_ref2) {
      ...
      const btnProps = {
        color: itemState.value[index2]?.color,
        density: props.density,
        disabled: props.disabled,
        icon: itemState.value[index2]?.icon,
        ripple: props.ripple,
        size: props.size,
        variant: "plain"
      };
    ...
    useRender(() => {
      ...
      return createVNode(VSheet, mergeProps(sheetProps, {
        ...
        default: () => [hasHeader && createVNode(VStepperHeader, {
          ...
          default: () => [items.value.map((_ref2, index2) => {
            let {
              raw,
              ...item
            } = _ref2;
            return createVNode(Fragment, null, [!!index2 && createVNode(VDivider, null, null), createVNode(VStepperItem, item, {
  ...
  setup(props) {
    ...
    useRender(() => {
      return createVNode(VSheet, {
        ...
        "color": props.event?.color ?? void 0,
  ...
  setup(props, _ref) {
    ...
    useRender(() => {
      ...
      return createVNode("div", {
        ...
        "class": props.day?.isToday ? "v-calendar-weekly__day-label__today" : void 0,
        "color": props.color,
        "disabled": props.disabled,
        "icon": true,
        "size": "x-small",
        "variant": props.day?.isToday ? void 0 : "flat"
      }, {
        default: () => [props.title]
...
var VFileUpload = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    function onClickRemove(index2) {
      model.value = model.value.filter((_, i) => i !== index2);
...
var VStepperVerticalItem = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isValid2 = computed(() => isSelected.value ? props.rules.every((handler) => handler() === true) : null);
    const canEdit = computed(() => !props.disabled && props.editable);
    const hasError = computed(() => props.error || isSelected.value && !isValid2.value);
    const hasCompleted = computed(() => props.complete || props.rules.length > 0 && isValid2.value === true);
    const disabled = computed(() => {
      if (props.disabled)
        return props.disabled;
