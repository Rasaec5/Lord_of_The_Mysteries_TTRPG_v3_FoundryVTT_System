...
export default class LOTM_VTTChatCard {
    
    static activateListeners(html) {
        html.on("click", ".collapsible", LOTM_VTTChatCard._onChatCardToggleCollapsible.bind(this));
        html.on("click", ".action", LOTM_VTTChatCard._handleActionClick.bind(this));
        html.on("click", ".revert-target-damage", LOTM_VTTChatCard._onRevertTargetDamage.bind(this));
        ...
        function applyMenus(roll) {
                ...
                for ( const target of targets ) {
                    ...
                    switch ( target.actor.type ) {
                        case 'pc':
                            ...
                            if ( type === 'temp' ) {
                                update['system.hp.temp'] = target.actor.system.hp.temp + (finalDamage !== undefined ? finalDamage : roll);
                                break;
                            }
                            ...
                            if ( type === 'damage' && target.actor.system.hp.temp > 0 ) {
                                ...
                                update['system.hp.temp'] = target.actor.system.hp.temp - appliedDamage;
                                ...
                                update['system.hp.value'] = target.actor.system.hp.value - (finalDamage !== undefined ? finalDamage : roll);
                            }
                            break;
                        case 'loot':
            ...
            async function createDamageApplicationChatCard(summary) {
                const setting = game.settings.get('LOTM_VTT', 'damageApplicationChatCard');
                ...
                menuItems.push({
                    ...
                    callback: (inlineRoll, event) => {
                        ...
                        if (button?.dataset?.target) {
                            ...
                            game.settings.set('LOTM_VTT', 'userTargetDamageApplicationType', button.dataset.target);
                        }
                    }
                });
            }
